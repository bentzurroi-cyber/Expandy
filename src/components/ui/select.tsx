...
>(function SelectTrigger({ className, children, ...props }, ref) {
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      dir={dir}
      className={cn(
        "flex min-h-11 w-full items-center justify-between rounded-xl border border-input bg-background px-3.5 py-2.5 text-start text-base leading-relaxed ring-offset-background focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1 [&_span[data-placeholder]]:text-muted-foreground",
        className,
      )}
      {...props}
    >
...
>(({ className, ...props }, ref) => (
  <SelectPrimitive.ScrollUpButton
    ref={ref}
    className={cn(
      "flex cursor-default items-center justify-center rounded-lg py-2 text-muted-foreground hover:bg-accent/40",
      className,
    )}
    {...props}
  >
...
>(function SelectContent(
  { className, children, position = "popper", align = "end", sideOffset = 4, ...props },
  ref,
) {
  const { dir } = useI18n();
  return (
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ...
    {...props}
  >
    <span className="absolute start-3 top-1/2 flex h-4 w-4 -translate-y-1/2 items-center justify-center text-primary">
