-- Ensure approving a join request is update-only on the existing row.
-- This avoids any insert/upsert path that can violate unique constraints on
-- household_join_requests (e.g. requester + household unique keys).
...
begin
  ...
  -- Lock the existing pending request row. Approval must operate on this row only.
  select
    r.id,
    r.requester_id,
    r.household_code,
  ...
  if v_req.id is null then
    -- Idempotent behavior for already-resolved requests.
    if exists (
      select 1
      from public.household_join_requests r
      where r.id = p_request_id
  ...
  -- Always: approver-owned categories are normalized to the target household.
  ...
  if v_import then
    ...
    -- Re-point requester's expenses to canonical categories in target household.
    update public.expenses e
    set
      category = (ex.id)::text,
    ...
    -- No-import: hard delete requester's old household data before switching
    -- them into the target household (clean slate join).
  ...
  -- Ensure requester joins the target household.
  ...
  -- Update existing request row only (never insert/upsert).
